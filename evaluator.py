...
def eval(model, ref_image, screenshot_image):
    ...
    "You are comparing two images. The first is a real reference image, and the second is a turtle graphics drawing done in Python. \
    ...
    Avoid vague statements and only mention corrections that could realistically be implemented using Python turtle (e.g., 'add more petals',\
    'change flower center color to yellow'). Check the position and size of the circle at the center of the flower. ",
        ref_image,
        screenshot_image
    ])
